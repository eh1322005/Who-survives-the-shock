...
-- =============================================
-- SECTION 1: VIEWS (Dimensions + Fact Tables)
-- =============================================


-- Country dimension view
-- I'm pulling all unique countries from all 15 source tables
-- then filtering to only the 100 countries in my study
...
-- Indicator dimension view
-- maps each World Bank indicator code to one of the 6 domains

CREATE OR REPLACE VIEW dim_indicator AS
SELECT DISTINCT
    `Indicator Code`,
    `Indicator Name`,
    CASE
...
-- Year dimension view
-- adds a decade label for trend grouping
...
-- Raw fact view -- stacks all 15 source tables into one place

CREATE OR REPLACE VIEW raw_fact AS
...
CREATE OR REPLACE VIEW minmax_per_indicator AS
SELECT
    f.`Indicator Code`,
...
-- Main fact table -- this is the core of the project
-- Joins everything together and calculates the normalized score for each row
-- Some indicators are "inverted" because higher values actually mean worse performance
-- (e.g. high inflation is bad, so we flip the score)

CREATE OR REPLACE VIEW fact_global_indicators AS
SELECT
    f.`Country Code`,
    f.`Indicator Code`,
    f.`Year`,
...
-- Food price index view -- unpivot the FAO table from wide to long format
-- original table has one row per month with 6 price columns

CREATE OR REPLACE VIEW fact_food_index AS
...
FROM fao_food_price_index
WHERE YEAR(STR_TO_DATE(`Date`, '%d/%m/%Y')) NOT IN (2024,2025,2026);
-- Lookup table for FAO commodity types

CREATE TABLE IF NOT EXISTS dim_type (
    type_key   TINYINT NOT NULL AUTO_INCREMENT PRIMARY KEY,
