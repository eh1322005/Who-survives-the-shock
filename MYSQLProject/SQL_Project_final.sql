...
FROM (
    SELECT f.country_key, STDDEV_POP(domain_avg) AS domain_stddev
    FROM (
...
FROM (
    SELECT
        ...
        ROUND(AVG(CASE WHEN d.Domain = 'Climate & Energy' AND f.`Indicator Code` != 'EG.USE.ELEC.KH.PC' THEN f.`Normalized Value` END) * 100, 2) AS `Climate Score`,
